...
!function (a) {
    ...
    function e(a, e, f, g) {
        ...
        } else c(a, e, "error", b, {error: "FileReader_not_support_" + f})
    }
    ...
    function h(a, b) {
        if (a)if (a.isFile)a.file(function (c) {
            ...
            c.readEntries(function (a) {
                api.afor(a, function (a, c) {
                    h(c, function (c, e) {
                        c ? api.log(c) : d = d.concat(e), a ? a() : b(!1, d)
                    })
                })
            }, function (a) {
    ...
    var m = 1, n = function () {
        ...
        if (a) {
            var d = api.uid(a), e = K[d] || {}, f = u && a && a instanceof u;
            R(b.split(/\s+/), function (b) {
    ...
    api = {
        ...
        uid: function (a) {
            return a ? a[api.expando] = a[api.expando] || api.uid() : (++m, api.expando + m)
        },
        log: function () {
            api.debug && api._supportConsoleLog && (api._supportConsoleLogApply ? console.log.apply(console, arguments) : console.log([].join.call(arguments, " ")))
        },
        newImage: function (a, b) {
            var c = o.createElement("img");
            return b && api.event.one(c, "error load", function (a) {
        ...
        readAsDataURL: function (a, b) {
            api.isCanvas(a) ? c(a, b, "load", api.toDataURL(a)) : e(a, b, "DataURL")
        },
        readAsBinaryString: function (a, b) {
            d("BinaryString") ? e(a, b, "BinaryString") : e(a, function (a) {
                if ("load" == a.type)try {
                    a.result = api.toBinaryString(a.result)
                } catch (c) {
        ...
        toBinaryString: function (b) {
            return a.atob(api.toDataURL(b).replace(G, ""))
        },
        readAsImage: function (a, d, e) {
            if (api.isFile(a))if (r) {
                var f = r.createObjectURL(a);
                f === b ? c(a, d, "error") : api.readAsImage(f, d, e)
            } else api.readAsDataURL(a, function (b) {
                "load" == b.type ? api.readAsImage(b.result, d, e) : (e || "error" == b.type) && c(a, d, b, null, {
                    loaded: b.loaded,
                    total: b.total
                })
            }); else if (api.isCanvas(a))c(a, d, "load", a); else if (C.test(a.nodeName))if (a.complete)c(a, d, "load", a); else {
                var g = "error abort load";
                V(a, g, function i(b) {
                    "load" == b.type && r && r.revokeObjectURL(a.src), U(a, g, i), c(a, d, b, a)
                })
            } else if (a.iframe)c(a, d, {type: "error"}); else {
                var h = api.newImage(a.dataURL || a);
                api.readAsImage(h, d, e)
            }
        },
        checkFileObj: function (a) {
            ...
            return "object" == typeof a ? b = a : b.name = (a + "").split(/\\|\//g).pop(), null == b.type && (b.type = b.name.split(".").pop()), R(c, function (a, c) {
                a = new RegExp(a.replace(/\s/g, "|"), "i"), (a.test(b.type) || api.ext2mime[b.type]) && (b.type = api.ext2mime[b.type] || c.split("/")[0] + "/" + b.type)
            }), b
        },
        getDropFiles: function (a, b) {
            var c = [], d = k(a), e = Q(d.items) && d.items[0] && g(d.items[0]), i = api.queue(function () {
                b(c)
            });
            R((e ? d.items : d.files) || [], function (a) {
                ...
                try {
                    e ? h(a, function (a, b) {
                        a ? api.log("[err] getDropFiles:", a) : c.push.apply(c, b), i.next()
                    }) : f(a, function (b) {
                        b && c.push(a), i.next()
                    })
                } catch (b) {
                    i.next(), api.log("[err] getDropFiles: ", b)
                }
            }), i.check()
        },
        getFiles: function (a, b, c) {
            var d = [];
            return c ? (api.filterFiles(api.getFiles(a), b, c), null) : (a.jquery && (a.each(function () {
                d = d.concat(api.getFiles(this))
            }), a = d, d = []), "string" == typeof b && (b = api.getFilesFilter(b)), a.originalEvent ? a = W(a.originalEvent) : a.srcElement && (a = W(a)), a.dataTransfer ? a = a.dataTransfer : a.target && (a = a.target), a.files ? (d = a.files, y || (d[0].blob = a, d[0].iframe = !0)) : !y && j(a) ? api.trim(a.value) && (d = [api.checkFileObj(a.value)], d[0].blob = a, d[0].iframe = !0) : Q(a) && (d = a), api.filter(d, function (a) {
        ...
        filterFiles: function (a, b, c) {
            if (a.length) {
                ...
                !function h() {
                    e.length ? (d = e.shift(), api.getInfo(d, function (a, c) {
        ...
        upload: function (a) {
            ...
            var b, c = new api.XHR(a), d = this._getFilesDataArray(a.files), e = this, f = 0, g = 0, h = !1;
            return R(d, function (a) {
                ...
                if (c.filesLeft = d.length, k && k.name === api.expando && (k = null, api.log("[warn] FileAPI.upload() — called without files")), ("abort" != c.statusText || c.current) && j) {
                    if (h = !1, c.currentFile = k, k && a.prepare(k, m) === !1)return void b.call(e);
                    m.file = k, e._getFormData(m, j, function (h) {
                        g || a.upload(c, a);
                        var i = new api.XHR(S({}, m, {
                            upload: k ? function () {
                                a.fileupload(k, i, m)
                ...
                a = api._getFilesDataArray([].concat(a)), R(a, function (a) {
        ...
        _getFilesDataArray: function (a) {
            var b = [], c = {};
            if (j(a)) {
                var d = api.getFiles(a);
                c[a.name || "file"] = null !== a.getAttribute("multiple") ? d : d[0]
            } else Q(a) && j(a[0]) ? R(a, function (a) {
                c[a.name || "file"] = api.getFiles(a)
            }) : c = a;
            return R(c, function e(a, c) {
                Q(a) ? R(a, function (a) {
                    e(a, c)
                }) : a && (a.name || a.image) && b.push({name: c, file: a, size: a.size, total: a.size, loaded: 0})
            }), b.length || b.push({file: {name: api.expando}}), b
        },
        _getFormData: function (a, b, c) {
            var d = b.file, e = b.name, f = d.name, g = d.type, h = api.support.transform && a.imageTransform, i = new api.Form, j = api.queue(function () {
                c(i)
            }), k = h && l(h), m = api.postNameConcat;
            R(a.data, function n(a, b) {
                ...
                })) : api.Image && h && (/^image/.test(b.type) || E.test(b.nodeName)) ? (j.inc(), k && (h = [h]), api.Image.transform(b, h, a.imageAutoOrientation, function (c, d) {
                    if (k && !c)B || api.flashEngine || (i.multipart = !0), i.append(e, d[0], f, h[0].type || g); else {
                        var l = 0;
                        c || R(d, function (a, b) {
                            B || api.flashEngine || (i.multipart = !0), h[b].postName || (l = 1), i.append(h[b].postName || m(e, b), a, f, h[b].type || g)
                        }), (c || a.imageOriginal) && i.append(m(e, l ? "original" : null), b, f, g)
                    }
                    j.next()
                })) : f !== api.expando && i.append(e, b, f)
            }(d), j.check()
        },
        reset: function (a, b) {
            var c, d;
            return x ? (d = x(a).clone(!0).insertBefore(a).val("")[0], b || x(a).remove()) : (c = a.parentNode, d = c.insertBefore(a.cloneNode(!0), a), d.value = "", b || c.removeChild(a), R(K[api.uid(a)], function (b, c) {
                R(b, function (b) {
                    U(a, c, b), T(d, c, b)
                })
            })), d
        ...
        load: function (a, b) {
            ...
            return c ? (c.open("GET", a, !0), c.overrideMimeType && c.overrideMimeType("text/plain; charset=x-user-defined"), T(c, "progress", function (a) {
                ...
                if (4 == c.readyState)if (c.onreadystatechange = null, 200 == c.status) {
                    ...
                    d.dataURL = "data:" + d.type + ";base64," + api.encode64(c.responseBody || c.responseText), b({
                        type: "load",
                        result: d
                    }, c)
                } else b({type: "error"}, c)
    ...
    }, api.addInfoReader(/^image/, function (a, b) {
        if (!a.__dimensions) {
            var c = a.__dimensions = api.defer();
            api.readAsImage(a, function (a) {
                var b = a.target;
                c.resolve("load" == a.type ? !1 : "error", {
                    width: b.width,
                    height: b.height
                }), b.src = api.EMPTY_PNG, b = null
            })
        }
        a.__dimensions.then(b)
    }), api.event.dnd = function (a, b, c) {
        var d, e;
        c || (c = b, b = api.F), u ? (T(a, "dragenter dragleave dragover", b.ff = b.ff || function (a) {
                ...
                a[P](), e = 0, b.call(a[O], !1, a), api.getDropFiles(a, function (b) {
                    c.call(a[O], b, a)
                })
            })) : api.log("Drag'n'Drop -- not supported")
    }, api.event.dnd.off = function (a, b, c) {
        U(a, "dragenter dragleave dragover", b.ff), U(a, "drop", c.ff)
    }, x && !x.fn.dnd && (x.fn.dnd = function (a, b) {
        return this.each(function () {
            api.event.dnd(this, a, b)
        })
    }, x.fn.offdnd = function (a, b) {
        return this.each(function () {
            api.event.dnd.off(this, a, b)
        })
    }), a.FileAPI = S(api, a.FileAPI), api.log("FileAPI: " + api.version), api.log("protocol: " + a.location.protocol), api.log("doctype: [" + p.name + "] " + p.publicId + " " + p.systemId), R(o.getElementsByTagName("meta"), function (a) {
        /x-ua-compatible/i.test(a.getAttribute("http-equiv")) && api.log("meta.http-equiv: " + a.getAttribute("content"))
    });
    try {
        api._supportConsoleLog = !!console.log, api._supportConsoleLogApply = !!console.log.apply
    } catch (Y) {
    }
    api.flashUrl || (api.flashUrl = api.staticPath + "FileAPI.flash.swf"), api.flashImageUrl || (api.flashImageUrl = api.staticPath + "FileAPI.flash.image.swf"), api.flashWebcamUrl || (api.flashWebcamUrl = api.staticPath + "FileAPI.flash.camera.swf")
    ...
    c.support.flash = function () {
        ...
        if (f.plugins && "object" == typeof f.plugins["Shockwave Flash"])d = f.plugins["Shockwave Flash"].description && !(b && b["application/x-shockwave-flash"] && !b["application/x-shockwave-flash"].enabledPlugin); else try {
